...
template <typename T_, R_, C_>
conv(Image<T_, R_, C_>& im, const Kernel<3, 3>& kern)
{
    for (int i = 1; i < im.r() - 1)
    {
        for (int j = 1; j < im.c() - 1)
        {
            T_ ch1_accum = 0;
            T_ ch2_accum = 0;
            T_ ch3_accum = 0;
...
// No border processing for now
template <typename T_, R_, C_>
conv(const Image<T_, R_, C_>& im, const Kernel<3, 3>& kern, Image<T_, R_, C_>& out)
{
    for (int i = 1; i < im.r() - 1)
    {
        for (int j = 1; j < im.c() - 1)
